...
const Nav = ({ history }) => {
    ...
    return (
        <nav ref={Nav} className={`nav ${nav ? 'active' : ''}`} >
            <div className="logo"><Link to=''>KAPDA BAZAR</Link></div>
            <ul className="navLinks" ref={navLinks}>
                ...
                {userInfo && userInfo.isVerified ? (
                    userInfo.passwordMessage ? (
                        <NavLink to="/profile" style={{ color: 'blue', fontSize: '16px', fontWeight: 'bold' }}>
                            {userInfo.passwordMessage}
                        </NavLink>
                    ) : (
                        <p></p>
                    ...
                    userInfo ? ( // <-- Additional check for userInfo
                        <NavLink to="/verify-email" style={{ color: 'blue', fontSize: '16px', fontWeight: 'bold' }}>
                            Please Verify Email
                        </NavLink>
                    ) : null
                )}
