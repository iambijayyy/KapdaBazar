...
const ProfileScreen = ({ location, history }) => {
  ...
  const submitHandler = (e) => {
    ...
    if (password !== confirmPassword) {
      ...
      setMessage('Password must contain at least 8 characters including uppercase, lowercase, digit, and special characters.');
    } else if (password && (password.includes(user.name) || password.includes(user.email))) {
      setMessage('Password cannot contain your name or email.');
    } else {
      if (!userInfo.isVerified) {
        setMessage('Please verify your account to update your profile.');
        return;
      }
      dispatch(updateUserProfile({ id: user._id, name, email, password }));
    }
  };
  // Clear the message state after 5 seconds
  useEffect(() => {
    let timer;
    if (message) {
      timer = setTimeout(() => {
